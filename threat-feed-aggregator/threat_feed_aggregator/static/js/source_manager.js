...
function onSrcFormatChange() {
    const fmt = document.getElementById('srcFormat').value;
    const isSgb = fmt === 'sgb';
    document.getElementById('srcSgbNote').style.display = isSgb ? 'block' : 'none';
    document.getElementById('srcManualFields').style.display = isSgb ? 'none' : 'block';
    const urlInput = document.getElementById('srcUrl');
    if (isSgb) {
        urlInput.value = 'https://siberguvenlik.gov.tr/api/address/index';
        urlInput.readOnly = true;
    } else {
        if (urlInput.value === 'https://siberguvenlik.gov.tr/api/address/index') {
            urlInput.value = '';
        }
...
function onEditFormatChange() {
    const fmt = document.getElementById('eFormat').value;
    const isSgb = fmt === 'sgb';
    document.getElementById('eSgbNote').style.display = isSgb ? 'block' : 'none';
    document.getElementById('eManualFields').style.display = isSgb ? 'none' : 'block';
...
function showAddSourceModal(submitUrl) {
    ...
    Swal.fire({
        ...
        html: `
            ...
            <div class="text-start mb-2"><label class="small fw-bold">URL</label><input type="text" id="srcUrl" class="form-control" placeholder="https://..."></div>
            <div class="row g-2 mb-2">
                <div class="col-6"><label class="small fw-bold">Format</label>
                    <select id="srcFormat" class="form-select" onchange="onSrcFormatChange()">
                        <option value="text">Text</option>
                        <option value="json">JSON</option>
                        <option value="csv">CSV</option>
                        <option value="sgb">SGB (Siber Güvenlik Başkanlığı)</option>
                    </select>
                </div>
            ...
            <div id="srcSgbNote" class="alert alert-info py-2 px-3 mb-2 small" style="display:none">
                <i class="fas fa-info-circle me-1"></i> <strong>SGB</strong> seçildi — URL ve tüm API ayarları (sayfalama, alan çıkarımı) otomatik yapılandırılacak. Sadece <em>kaynak ismi</em> ve <em>güven skoru</em> girin.
            </div>
            <div id="srcManualFields">
        ...
        preConfirm: () => {
            ...
            if (!url && fmt !== 'sgb') { Swal.showValidationMessage('URL is required'); return; }
            const isApi = fmt === 'sgb' ? false : document.getElementById('srcApiMode').checked;
            const result = {
                name,
                url: url || 'https://siberguvenlik.gov.tr/api/address/index',
                format: fmt,
            ...
            if (fmt !== 'sgb') {
                result.key_or_column = document.getElementById('srcKey').value;
                result.auth_user = document.getElementById('srcAuthUser').value;
                result.auth_pass = document.getElementById('srcAuthPass').value;
                if (isApi) {
...
function showEditSourceModal(index, source, submitUrlBase) {
    ...
    const isApiMode = source.fetch_type === 'api' || source.format === 'sgb';
    const isSgb = source.format === 'sgb';
    ...
    Swal.fire({
        ...
        html: `
            ...
            <div class="text-start mb-2"><label class="small fw-bold">URL</label><input type="text" id="eUrl" class="form-control" value="${source.url}" ${isSgb ? 'readonly' : ''}></div>
            <div class="row g-2 mb-2">
                <div class="col-6"><label class="small fw-bold">Format</label>
                    <select id="eFormat" class="form-select" onchange="onEditFormatChange()">
                        <option value="text" ${source.format === 'text' ? 'selected' : ''}>Text</option>
                        <option value="json" ${source.format === 'json' ? 'selected' : ''}>JSON</option>
                        <option value="csv" ${source.format === 'csv' ? 'selected' : ''}>CSV</option>
                        <option value="sgb" ${source.format === 'sgb' ? 'selected' : ''}>SGB (Siber Güvenlik Başkanlığı)</option>
                    </select>
                </div>
            ...
            <div id="eSgbNote" class="alert alert-info py-2 px-3 mb-2 small" style="display:${isSgb ? 'block' : 'none'}">
                <i class="fas fa-info-circle me-1"></i> <strong>SGB</strong> modu aktif — URL ve API ayarları otomatik yönetilir.
            </div>
            <div id="eManualFields" style="display:${isSgb ? 'none' : 'block'}">
                ...
                <div class="text-start mb-2">
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="eApiMode" ${isApiMode && !isSgb ? 'checked' : ''} onchange="toggleApiFields('e')">
                        <label class="form-check-label small fw-bold" for="eApiMode">API Mode</label>
                    </div>
                </div>
                <div id="eApiFields" style="display:${isApiMode && !isSgb ? 'block' : 'none'}">
        ...
        preConfirm: () => {
            ...
            const isApi = fmt === 'sgb' ? false : document.getElementById('eApiMode').checked;
            const result = {
                name, url,
                format: fmt,
            ...
            if (fmt !== 'sgb') {
                result.key_or_column = document.getElementById('eKey').value;
                result.auth_user = document.getElementById('eAuthUser').value;
                result.auth_pass = document.getElementById('eAuthPass').value;
                if (isApi) {
...
window.onSrcFormatChange = onSrcFormatChange;
window.onEditFormatChange = onEditFormatChange;
