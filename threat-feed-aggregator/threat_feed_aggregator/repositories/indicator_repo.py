...
def get_filter_options(column, search_term=None, limit=20, conn=None):
    with db_readonly() as db:
        ...
        if column == 'source':
            return [row[0] for row in db.execute("SELECT DISTINCT source_name FROM indicator_sources WHERE source_name LIKE ? ORDER BY source_name LIMIT ?", (p, limit)).fetchall()]
        if column == 'country':
            return [row[0] for row in db.execute("SELECT DISTINCT country FROM indicators WHERE country LIKE ? ORDER BY country LIMIT ?", (p, limit)).fetchall() if row[0]]
        if column == 'type':
            return [row[0] for row in db.execute("SELECT DISTINCT type FROM indicators WHERE type LIKE ? ORDER BY type LIMIT ?", (p, limit)).fetchall()]
        return []
