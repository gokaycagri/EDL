...
def get_all_indicators(conn=None):
    with db_transaction(conn) as db:
        ...
        return {row['indicator']: {
            'last_seen': row['last_seen'], 'country': row['country'], 'type': row['type'],
            'risk_score': row['risk_score'], 'source_count': row['source_count']
...
def get_unique_indicator_count(itype=None, conn=None):
    def _fetch(t, c):
        with db_transaction(c) as db:
            if t: return db.execute('SELECT COUNT(*) FROM indicators WHERE type = ?', (t,)).fetchone()[0]
...
def get_indicator_counts_by_type(conn=None):
    def _fetch(c):
        with db_transaction(c) as db:
            return {row['type']: row['count'] for row in db.execute('SELECT type, COUNT(*) as count FROM indicators GROUP BY type').fetchall()}
    return _get_cached_stat("counts_by_type", _fetch, conn)

def get_country_stats(conn=None):
    def _fetch(c):
        with db_transaction(c) as db:
            return [dict(row) for row in db.execute("SELECT COALESCE(country, 'Unknown') as country_code, COUNT(*) as count FROM indicators WHERE type = 'ip' GROUP BY country_code ORDER BY count DESC LIMIT 250").fetchall()]
...
def get_historical_stats(days=30, conn=None):
    with db_transaction(conn) as db:
        ...
        return [dict(row) for row in db.execute('SELECT * FROM stats_history WHERE timestamp > ? ORDER BY timestamp ASC', (cutoff,)).fetchall()]

def get_source_counts(conn=None):
    def _fetch(c):
        with db_transaction(c) as db:
            return {row['source_name']: row['count'] for row in db.execute('SELECT source_name, COUNT(*) as count FROM indicator_sources GROUP BY source_name').fetchall()}
    return _get_cached_stat("source_counts", _fetch, conn)

def get_sources_for_indicator(indicator, conn=None):
    with db_transaction(conn) as db:
        return [dict(row) for row in db.execute('SELECT source_name, last_seen FROM indicator_sources WHERE indicator = ? ORDER BY last_seen DESC', (indicator,)).fetchall()]
...
def get_sources_for_indicators_batch(indicators, conn=None):
    ...
    with db_transaction(conn) as db:
        ...
        for row in db.execute(f'SELECT indicator, source_name, last_seen FROM indicator_sources WHERE indicator IN ({placeholders})', indicators).fetchall():
    ...
    with db_transaction(conn) as db:
        ...
        if filters:
            for col, val in filters.items():
                if not val: continue
                if col == 'type': conds.append("i.type = ?"); params.append(val)
                elif col == 'country': conds.append("i.country LIKE ?"); params.append(f"%{val}%")
                elif col == 'level':
                    if val == 'Critical': conds.append("i.risk_score >= 90")
                    elif val == 'High': conds.append("i.risk_score >= 70 AND i.risk_score < 90")
                    elif val == 'Medium': conds.append("i.risk_score >= 40 AND i.risk_score < 70")
                    elif val == 'Low': conds.append("i.risk_score < 40")
                elif col == 'risk_score':
                    v = str(val).strip()
                    op = ">="
                    if v.startswith('>='): op, v = ">=", v[2:]
                    elif v.startswith('<='): op, v = "<=", v[2:]
                    ...
                    if "JOIN indicator_sources s" not in joins: joins.append("JOIN indicator_sources s ON i.indicator = s.indicator")
                    vl = val.lower()
                    if 'botnet' in vl: conds.append("s.source_name LIKE '%feodo%'")
                    elif 'malware' in vl: conds.append("s.source_name LIKE '%urlhaus%'")
                    else: conds.append("s.source_name LIKE ?"); params.append(f"%{val}%")

        if joins: j = " " + " ".join(joins); base_query += j; count_query += j
        if conds: w = " WHERE " + " AND ".join(conds); base_query += w; count_query += w
...
def get_existing_ips(ip_list, conn=None):
    ...
    with db_transaction(conn) as db:
        ...
        for i in range(0, len(ip_list), 900):
            ...
            p = ','.join(['?'] * len(chunk))
            existing.update(row[0] for row in db.execute(f"SELECT indicator FROM indicators WHERE type='ip' AND indicator IN ({p})", chunk).fetchall())
        return existing
