...
class TestCustomEDL(unittest.TestCase):
    ...
    def test_create_and_get_custom_list(self):
        ...
        list_id, token = create_custom_list(name, sources, types, fmt, conn=None)
        
        # Verify get_all
        lists = get_all_custom_lists(conn=None)
        ...
        # Verify get_by_token
        fetched = get_custom_list_by_token(token, conn=None)
        self.assertIsNotNone(fetched)
        self.assertEqual(fetched['id'], list_id)

    def test_delete_custom_list(self):
        list_id, _ = create_custom_list("Del List", [], [], "text", conn=None)
        self.assertTrue(delete_custom_list(list_id, conn=None))
        self.assertEqual(len(get_all_custom_lists(conn=None)), 0)
    ...
    def test_internal_search_logic(self):
        ...
        upsert_indicators_bulk(indicators, source_name="Feed A", conn=None)
        
        # Add duplicate to test history
        upsert_indicators_bulk([("1.1.1.1", "US", "ip")], source_name="Feed B", conn=None)

        # 2. Test get_sources_for_indicator
        sources = get_sources_for_indicator("1.1.1.1", conn=None)
        self.assertEqual(len(sources), 2)
        names = sorted([s['source_name'] for s in sources])
        self.assertEqual(names, ["Feed A", "Feed B"])

        # Test non-existent
        sources = get_sources_for_indicator("9.9.9.9", conn=None)
        self.assertEqual(len(sources), 0)

    def test_filtered_indicators_iter(self):
        # Seed
        upsert_indicators_bulk([("1.1.1.1", "US", "ip")], source_name="Src1", conn=None)
        upsert_indicators_bulk([("2.2.2.2", "US", "ip")], source_name="Src2", conn=None)
        upsert_indicators_bulk([("3.3.3.3", "US", "ip")], source_name="Src1", conn=None)

        # Test Filter
        iterator = get_filtered_indicators_iter(["Src1"], conn=None)
    ...
    def test_recalculate_scores(self):
        # Seed
        upsert_indicators_bulk([("1.1.1.1", "US", "ip")], source_name="HighConf", conn=None)
        upsert_indicators_bulk([("1.1.1.1", "US", "ip")], source_name="LowConf", conn=None)
        upsert_indicators_bulk([("2.2.2.2", "US", "ip")], source_name="LowConf", conn=None)

        conf_map = {"HighConf": 90, "LowConf": 10}
        
        # 1. Test Full Recalculation
        recalculate_scores(conf_map, conn=None)
        data = get_all_indicators(conn=None)
        ...
        recalculate_scores(conf_map, conn=None, target_source="HighConf")
