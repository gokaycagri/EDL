...
def test_lookup_internal_endpoint_integration(client, app):
    ...
    with (patch('threat_feed_aggregator.db_manager.get_sources_for_indicator', return_value=mock_global_sources.copy()),
          patch('threat_feed_aggregator.db_manager.get_api_blacklist_item_by_value', return_value=mock_blacklist_item)):
        ...
        # api_response wraps in {"status": "success", "data": {...}}
...
def test_lookup_internal_no_blacklist_match(client, app):
    ...
    with (patch('threat_feed_aggregator.db_manager.get_sources_for_indicator', return_value=mock_global_sources),
          patch('threat_feed_aggregator.db_manager.get_api_blacklist_item_by_value', return_value=None)):
        ...
        assert data['data']['sources'][0]['source_name'] == 'Google-Safe'
