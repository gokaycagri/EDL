...
class TestWhitelistLogic(unittest.TestCase):
    ...
    @patch('threat_feed_aggregator.aggregator.get_all_indicators_iter')
    @patch('threat_feed_aggregator.aggregator.db_delete_whitelisted_indicators')
    def test_cleanup_whitelisted_items_from_db(self, mock_delete, mock_get_indicators_iter, mock_get_whitelist):
        # Setup mocks
        mock_get_whitelist.return_value = [{'item': '10.0.0.0/8'}]
        # mock_get_indicators_iter should return an iterator of rows (dicts)
        mock_get_indicators_iter.return_value = iter([
            {'indicator': '203.0.113.1', 'type': 'ip'}, # Not in safe list
            {'indicator': '10.0.0.5', 'type': 'ip'},    # In user whitelist (CIDR)
            {'indicator': '10.1.2.3', 'type': 'ip'}     # In user whitelist (CIDR)
        ])
